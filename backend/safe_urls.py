...
def _origin(url: str):
    ...
    Normalisation : schéma en minuscules, hostname en minuscules, port
    effectif (80 pour http, 443 pour https si absent).
    Refuse userinfo, tout hôte/port mal formé, ainsi que tout hostname portant
    un point final (trailing-dot) : le FQDN absolu est ambigu pour le navigateur
    et ne doit JAMAIS être normalisé en même-origine — fail-safe.
    """
    ...
    try:
        ...
        if hostname.endswith("."):
            # hostname « joboolo.fr. » : même résolu par le navigateur, il
            # reste une forme ambiguë distincte — on ne l'accepte jamais.
            return None
        port = parts.port
