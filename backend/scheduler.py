...
def _build_job_query(alert: dict, since: datetime, public_filter: dict) -> dict:
    ...
    visibilité public (public_filter). Les `$or` de l'alerte (search) sont
    placés dans `$and` : la visibilité publique ET les critères de l'alerte
    doivent tous deux s'appliquer (pas de ré-exposition des campagnes cachées)."""
