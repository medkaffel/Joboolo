...
async def main():
    ...
    try:
        malformed = await db.role_dnas.find_one({
            "$or": [
                {"role_dna_id": {"$exists": False}},
                {"role_dna_id": None},
        ...
        if malformed:
            raise SystemExit("malformed role_dnas detected; index not created")
        ...
        await db.role_dnas.create_index(
            [("role_dna_id", 1), ("version", 1)], unique=True, name=INDEX
        )
