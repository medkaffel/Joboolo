...
class ProfileService:
    ...
    LEGACY_MIRROR_FIELDS = {"summary": "bio", "current_location": "location", "experience_years": "experience_years"}
    ...
    async def _materialize_if_missing(self, candidate_id: str, session) -> dict:
        ...
        if current:
            return current
        user_doc = await self.db.users.find_one({"_id": candidate_id, "user_type": "candidate"}, session=session)
        if not user_doc:
            raise ProfileNotFoundError(candidate_id)
            ...
            async with session.start_transaction():
                ...
                if expected_version is not None and current_version != expected_version:
                    raise ProfileConflictError(
                        f"profile version mismatch: expected {int(expected_version)}, current {int(current_version)}"
                    )
