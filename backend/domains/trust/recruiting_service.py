...
from .organization_models import OrganizationVerificationState
...
def _values(values): return [value.value if hasattr(value, "value") else value for value in values]
...
def _membership(doc):
    if doc.get("_id") != doc.get("membership_id"): raise ValueError("membership identity mismatch")
    return OrganizationMembership(
        membership_id=MembershipId(doc["membership_id"]), recruiter_user_id=RecruiterUserId(doc["recruiter_user_id"]),
        organization_id=OrganizationId(doc["organization_id"]), version=EntityVersion(int(doc["version"])),
        role=MembershipRole(doc["role"]),
        state=MembershipState(doc["state"]), policy_version=PolicyVersion(doc["policy_version"]),
        reason_codes=tuple(MembershipReasonCode(v) for v in doc["reason_codes"]), evidence_refs=tuple(doc.get("evidence_refs", [])),
        actor_id=doc["actor_id"], decided_at=doc["decided_at"], created_at=doc["created_at"], updated_at=doc["updated_at"])


def _verification(doc):
    if doc.get("_id") != doc.get("recruiter_user_id"): raise ValueError("recruiter verification identity mismatch")
    return RecruiterVerification(
        recruiter_user_id=RecruiterUserId(doc["recruiter_user_id"]), version=EntityVersion(int(doc["version"])),
        state=RecruiterVerificationState(doc["state"]), policy_version=PolicyVersion(doc["policy_version"]),
        reason_codes=tuple(RecruiterVerificationReasonCode(v) for v in doc["reason_codes"]), evidence_refs=tuple(doc.get("evidence_refs", [])),
        actor_id=doc["actor_id"], decided_at=doc["decided_at"], created_at=doc["created_at"], updated_at=doc["updated_at"])


def _mandate(doc):
    if doc.get("_id") != doc.get("mandate_id"): raise ValueError("mandate identity mismatch")
    return RecruitingMandate(
        mandate_id=MandateId(doc["mandate_id"]), requesting_organization_id=OrganizationId(doc["requesting_organization_id"]),
        hiring_company_id=HiringCompanyId(doc["hiring_company_id"]), version=EntityVersion(int(doc["version"])),
        state=MandateState(doc["state"]), valid_from=doc["valid_from"], valid_until=doc["valid_until"],
        policy_version=PolicyVersion(doc["policy_version"]), reason_codes=tuple(MandateReasonCode(v) for v in doc["reason_codes"]),
        evidence_refs=tuple(doc.get("evidence_refs", [])), actor_id=doc["actor_id"], decided_at=doc["decided_at"],
        created_at=doc["created_at"], updated_at=doc["updated_at"])


class RecruitingTrustService:
    def __init__(self, db, *, client_provider=get_client):
        self.repo = RecruitingTrustRepository(db); self.client_provider = client_provider

    def _client(self):
        client = self.client_provider()
        if client is None: raise RuntimeError("Mongo client unavailable")
        return client
    ...
    async def _eligible_user(self, recruiter_user_id: str, session=None):
        ...
        if user is None: raise RecruitingTrustInputNotFoundError("Recruiter user not found")
        if not user.get("is_active", True): raise RecruitingTrustEligibilityError("Recruiter user is inactive")
        if user.get("user_type") != "employer": raise RecruitingTrustEligibilityError("Recruiter user type is unsupported")
        return user

    async def _organization(self, organization_id: str, session=None):
        doc = await self.repo.get_organization(organization_id, session=session)
        if doc is None: raise RecruitingTrustInputNotFoundError("Organization not found")
        return doc

    async def create_membership(self, command: MembershipCreate, *, now: Optional[datetime]=None) -> OrganizationMembership:
        now = now or _utcnow(); client = self._client()
        try:
        ...
        except DuplicateKeyError as exc: raise RecruitingTrustConflictError("Membership already exists") from exc
    ...
    async def transition_membership(self, membership_id: MembershipId, expected_version: EntityVersion, transition: MembershipTransition, *, now: Optional[datetime]=None) -> OrganizationMembership:
        ...
        async with await client.start_session() as session:
            async with session.start_transaction():
                ...
                if current is None: raise RecruitingTrustInputNotFoundError("Membership not found")
                state = MembershipState(current["state"])
                if transition.new_state not in _MEMBERSHIP_TRANSITIONS[state]: raise ValueError(f"invalid membership transition {state.value}->{transition.new_state.value}")
                ...
                if updated is None: raise RecruitingTrustConflictError("Membership version conflict")
    ...
    async def create_recruiter_verification(self, command: RecruiterVerificationCreate, *, now: Optional[datetime]=None) -> RecruiterVerification:
        ...
        except DuplicateKeyError as exc: raise RecruitingTrustConflictError("Recruiter verification already exists") from exc
    ...
    async def transition_recruiter_verification(self, recruiter_user_id: RecruiterUserId, expected_version: EntityVersion, transition: RecruiterVerificationTransition, *, now: Optional[datetime]=None) -> RecruiterVerification:
        ...
        async with await client.start_session() as session:
            async with session.start_transaction():
                ...
                if current is None: raise RecruitingTrustInputNotFoundError("Recruiter verification not found")
                state = RecruiterVerificationState(current["state"])
                if transition.new_state not in _RECRUITER_TRANSITIONS[state]: raise ValueError(f"invalid recruiter verification transition {state.value}->{transition.new_state.value}")
                changes = {"state": transition.new_state.value, "policy_version": str(transition.policy_version), "reason_codes": _values(transition.reason_codes),
                           "evidence_refs": list(transition.evidence_refs), "actor_id": transition.actor_id, "decided_at": now, "updated_at": now}
                updated = await self.repo.update_recruiter_verification(str(recruiter_user_id), int(expected_version), changes, session=session)
                if updated is None: raise RecruitingTrustConflictError("Recruiter verification version conflict")
        ...
        try:
            async with await client.start_session() as session:
                async with session.start_transaction():
                    ...
                    return _mandate(doc)
        except DuplicateKeyError as exc: raise RecruitingTrustConflictError("Mandate already exists") from exc
    ...
    async def transition_mandate(self, mandate_id: MandateId, expected_version: EntityVersion, transition: MandateTransition, *, now: Optional[datetime]=None) -> RecruitingMandate:
        ...
        async with await client.start_session() as session:
            async with session.start_transaction():
                ...
                if current is None: raise RecruitingTrustInputNotFoundError("Mandate not found")
                state = MandateState(current["state"])
                if transition.new_state not in _MANDATE_TRANSITIONS[state]: raise ValueError(f"invalid mandate transition {state.value}->{transition.new_state.value}")
                if transition.new_state is MandateState.ACTIVE:
                    requesting = await self._organization(current["requesting_organization_id"], session=session)
                    hiring = await self._organization(current["hiring_company_id"], session=session)
                    if requesting.get("verification_state") != OrganizationVerificationState.VERIFIED.value or hiring.get("verification_state") != OrganizationVerificationState.VERIFIED.value:
                        raise RecruitingTrustEligibilityError("Both mandate organizations must be VERIFIED before activation")
                    if current["valid_until"] <= now: raise RecruitingTrustEligibilityError("Expired mandate cannot be activated")
                changes = {"state": transition.new_state.value, "policy_version": str(transition.policy_version), "reason_codes": _values(transition.reason_codes),
                           "evidence_refs": list(transition.evidence_refs), "actor_id": transition.actor_id, "decided_at": now, "updated_at": now}
                updated = await self.repo.update_mandate(str(mandate_id), int(expected_version), changes, session=session)
                if updated is None: raise RecruitingTrustConflictError("Mandate version conflict")
    ...
    async def evaluate_recruiting_actor_trust(self, context: RecruitingActorContext, *, policy_version: PolicyVersion, evaluated_at: Optional[datetime]=None) -> TrustDecision:
        ...
        if now.tzinfo is None or now.utcoffset() is None: raise ValueError("evaluated_at must be timezone-aware")
        recruiter = str(context.recruiter_user_id); requesting_id = str(context.requesting_organization_id); hiring_id = str(context.hiring_company_id)
        user = await self.repo.get_user(recruiter)
        if user is None: return self._decision(False, RecruitingTrustReasonCode.RECRUITER_USER_NOT_FOUND, policy_version, now)
        if not user.get("is_active", True): return self._decision(False, RecruitingTrustReasonCode.RECRUITER_USER_INACTIVE, policy_version, now)
        if user.get("user_type") != "employer": return self._decision(False, RecruitingTrustReasonCode.RECRUITER_USER_TYPE_UNSUPPORTED, policy_version, now)
        requesting = await self.repo.get_organization(requesting_id)
        if requesting is None or requesting.get("_id") != requesting_id or requesting.get("organization_id", requesting_id) != requesting_id or requesting.get("verification_state") != OrganizationVerificationState.VERIFIED.value:
            return self._decision(False, RecruitingTrustReasonCode.REQUESTING_ORGANIZATION_NOT_VERIFIED, policy_version, now)
        membership = await self.repo.get_membership_by_pair(recruiter, requesting_id)
        try: membership_obj = None if membership is None else _membership(membership)
        except (KeyError, TypeError, ValueError): membership_obj = None
        if membership_obj is None or membership_obj.state is not MembershipState.ACTIVE:
            return self._decision(False, RecruitingTrustReasonCode.MEMBERSHIP_NOT_ACTIVE, policy_version, now)
        verification = await self.repo.get_recruiter_verification(recruiter)
        try: verification_obj = None if verification is None else _verification(verification)
        except (KeyError, TypeError, ValueError): verification_obj = None
        if verification_obj is None or verification_obj.state is not RecruiterVerificationState.VERIFIED:
            return self._decision(False, RecruitingTrustReasonCode.RECRUITER_NOT_VERIFIED, policy_version, now)
        hiring = await self.repo.get_organization(hiring_id)
        if hiring is None or hiring.get("_id") != hiring_id or hiring.get("organization_id", hiring_id) != hiring_id or hiring.get("verification_state") != OrganizationVerificationState.VERIFIED.value:
            return self._decision(False, RecruitingTrustReasonCode.HIRING_COMPANY_NOT_VERIFIED, policy_version, now)
        evidence = (f"organization:{requesting_id}:v{requesting.get('version')}", f"membership:{membership_obj.membership_id}:v{membership_obj.version}",
                    f"recruiter_verification:{recruiter}:v{verification_obj.version}", f"organization:{hiring_id}:v{hiring.get('version')}")
        ...
        mandate = await self.repo.get_mandate(str(context.mandate_id))
        if mandate is None: return self._decision(False, RecruitingTrustReasonCode.MANDATE_NOT_FOUND, policy_version, now, evidence)
        try: mandate_obj = _mandate(mandate)
        except (KeyError, TypeError, ValueError): return self._decision(False, RecruitingTrustReasonCode.MANDATE_NOT_ACTIVE, policy_version, now, evidence)
        if str(mandate_obj.requesting_organization_id) != requesting_id or str(mandate_obj.hiring_company_id) != hiring_id:
            return self._decision(False, RecruitingTrustReasonCode.MANDATE_PARTIES_MISMATCH, policy_version, now, evidence)
        if mandate_obj.state is not MandateState.ACTIVE:
            return self._decision(False, RecruitingTrustReasonCode.MANDATE_NOT_ACTIVE, policy_version, now, evidence)
        if now < mandate_obj.valid_from: return self._decision(False, RecruitingTrustReasonCode.MANDATE_NOT_YET_VALID, policy_version, now, evidence)
        if now >= mandate_obj.valid_until: return self._decision(False, RecruitingTrustReasonCode.MANDATE_EXPIRED, policy_version, now, evidence)
        return self._decision(True, RecruitingTrustReasonCode.RECRUITING_ACTOR_TRUSTED, policy_version, now, evidence + (f"mandate:{mandate_obj.mandate_id}:v{mandate_obj.version}",))
