...
def _make_candidate(
    *,
    ...
    return StreamCandidate(
        ...
        role_dna_version=EntityVersion(role_dna_version),
        opportunity_spec_id=OpportunitySpecId("spec-1"),
        opportunity_spec_version=EntityVersion(opportunity_spec_version),
        computed_at=_utc(100),
        discovery_evidence=DiscoveryEvidence(
            candidate_preferences_version=EntityVersion(candidate_preferences_version),
            updated_at=_utc(50),
        ),
        professional_match_summary=ProfessionalMatchSummary(
            candidate_profile_version=EntityVersion(candidate_profile_version),
            role_dna_version=EntityVersion(match_role_dna_version if match_role_dna_version is not None else role_dna_version),
        ...
        opportunity_fit_summary=OpportunityFitSummary(
            candidate_preferences_version=EntityVersion(candidate_preferences_version),
            opportunity_spec_version=EntityVersion(fit_opportunity_spec_version if fit_opportunity_spec_version is not None else opportunity_spec_version),
