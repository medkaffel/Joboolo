...
def test_register_success_shape_and_duplicate_no_write(setup):
    ...
    assert_error(client.post('/api/auth/register', json=PAYLOAD),400,'Email already registered')
    assert len(db.users.writes) == 1


def test_register_validation_no_write(setup):
    client, db = setup
    r = client.post('/api/auth/register', json={**PAYLOAD,'email':'invalid'})
...
def test_login_success_and_invalid_credentials(setup):
    ...
    for email, password in [('absent@example.com','synthetic-password'),(PAYLOAD['email'],'wrong')]:
        assert_error(client.post('/api/auth/login',json={'email':email,'password':password}),401,'Incorrect email or password',True)
    assert db.users.writes == []
...
def test_me_auth_errors_no_write(setup,method,header,status,detail,bearer):
    client, db = setup
    assert_error(client.request(method,'/api/auth/me',headers=header,json={}),status,detail,bearer)
    assert db.users.writes == []
...
def test_me_get_and_identity_update_null_is_ignored(setup):
    ...
    r = client.put('/api/auth/me',headers=AUTH,json={'first_name':'Changed','phone':None,'user_type':'admin'})
    ...
    assert client.put('/api/auth/me',headers=AUTH,json={'phone':None}).status_code == 200
...
def test_inactive_login_and_me_have_different_status(setup):
    ...
    assert_error(client.post('/api/auth/login',json={'email':PAYLOAD['email'],'password':PAYLOAD['password']}),403,'Votre compte est en attente de validation par notre équipe.')
    assert_error(client.get('/api/auth/me',headers=AUTH),400,'Inactive user')
    assert db.users.writes == []
...
def test_register_lookup_collision_no_write(setup,monkeypatch):
    ...
    assert_error(client.post('/api/auth/register',json=PAYLOAD),503,'Email lookup temporarily unavailable, please retry')
    assert db.users.writes == []
