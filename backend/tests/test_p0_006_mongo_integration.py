...
offres entreprise, suggest, alertes, sauvegarde, candidature, recommandations
IA, match IA, clics, impressions, import manuel/auto, non-renouvellement de
expires_at et régressions P0-004/P0-005.
...
def _install_base_stubs(monkeypatch):
    ...
    for name in ("Job", "JobCreate", "JobUpdate", "JobResponse", "JobSearchQuery",
                 ...
                 "ApplicationCreate", "ApplicationResponse", "JobAlert", "JobAlertCreate",
                 "JobAlertUpdate", "JobAlertResponse"):
...
class TestPublicReaders:
    def _scenario(self, jobs_module):
        client = AsyncIOMotorClient(MONGO_URL, serverSelectionTimeoutMS=3000)
        db_name = f"p006_readers_{uuid.uuid4().hex}"
        db = client[db_name]
        try:
            await_ = _Awaited(db)
            jobs_module.get_database = lambda: await_

            # Campagne paused -> offre non visible
            ...
            # get_job : 404 sur campagne non diffusible, ok sur visible
            try:
            ...
            r = await jobs_module.get_job("job_active")
            assert r.id == "job_active"
            r2 = await jobs_module.get_job("job_legacy")
            assert r2.id == "job_legacy"
            ...
            assert "job_active" in ids and "job_legacy" in ids
            assert "job_paused" not in ids

            # suggest : exclut job_paused
            sug = await jobs_module.suggest(q="Dev", field="title")
            assert "Dev" in sug["suggestions"]
            # (job_paused est le seul avec titre Dev parmi actifs ? evitar conflit)
        finally:
    ...
    def test_public_readers_hide_non_diffusible(self, jobs_module):
        ...
        _run(self._scenario(jobs_module))
...
class TestClickGuard:
    def test_click_on_paused_campaign_404_no_debit(self, jobs_module):
        ...
        async def scenario():
            ...
            try:
                ...
                job_paused = _job_doc("job_paused", campaign_id="paused")
                job_active = _job_doc("job_active", campaign_id="active")
                await db.jobs.insert_one(job_paused)
                await db.jobs.insert_one(job_active)
    ...
    def test_budget_exhaustion_pauses_campaign_but_does_not_deactivate_jobs(self, jobs_module):
        ...
        async def scenario():
            ...
            db = client[db_name]
            try:
                await db.campaigns.insert_one(_camp_doc("active", budget_limit=0.5, spent=0.0))
                ...
                assert camp["status"] == "paused"  # mise en pause auto (spent=0.5 >= 0.5)
    ...
    def test_save_and_apply_reject_non_diffusible(self, saved_jobs_module, applications_module):
        ...
        async def scenario():
            ...
            try:
                ...
                # get_saved_jobs : exclut job_paused (même si sauvé) ; ici on ne l'a pas
    ...
    def test_manual_import_non_diffusible_409_no_write(self, feed_module):
        ...
        async def scenario():
            ...
            try:
                ...
                # aucune écriture : pas de job, pas de log, jobs_count inchangé
    ...
    def test_scheduler_auto_refresh_skips_non_diffusible(self, scheduler_module):
        ...
        async def scenario():
            ...
            try:
                ...
                await_ = _Awaited(db)
                scheduler_module.get_database = (lambda: await_)
