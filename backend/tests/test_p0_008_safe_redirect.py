...
Règle fail-safe P0-008 : un hostname portant un point final (trailing-dot) —
aussi bien sur la cible que sur l'APP_URL canonique — n'est JAMAIS normalisé en
même-origine : la cible est refusée `/`, et un APP_URL ambigu refuse les
absolus tout en laissant fonctionner les relatifs sûrs.
...
class TestRefused:
    @pytest.mark.parametrize("target", [
        ...
        "https://joboolo.fr./",                    # trailing-dot : jamais même-origine
        "https://joboolo.fr./jobs/123",            # idem, même avec l'APP sans point
        "https://JOBoolo.FR./jobs/123",            # trailing-dot + casse
        "//evil.example",
        "///evil.example",
        "////evil.example",
...
class TestTrailingDotAppUrl:
    def test_trailing_dot_app_url_refuses_absolutes(self):
        app = "https://joboolo.fr."
        assert _refuses("https://joboolo.fr./jobs/123", app)
        assert _refuses("https://joboolo.fr/jobs/123", app)  # même sans point côté cible

    def test_trailing_dot_app_url_keeps_safe_relatives(self):
        app = "https://joboolo.fr."
        assert safe_urls.safe_redirect("/jobs/123", app) == "/jobs/123"
        assert safe_urls.safe_redirect("/", app) == "/"

    def test_plain_app_url_refuses_trailing_dot_target(self):
        assert _refuses("https://joboolo.fr./jobs/123", "https://joboolo.fr")

    def test_dot_only_host_app_url_refuses_absolutes(self):
        app = "https://./"
        assert _refuses("https://joboolo.fr/x", app)
        assert safe_urls.safe_redirect("/profile", app) == "/profile"
...
class TestTrackHandler:
    ...
    def test_trailing_dot_absolute_refused_and_tracks(self, alerts_module, monkeypatch):
        db = _make_fake_db({"_id": "alert_1", "user_id": "user_1"})
