...
- une date `start_date`/`end_date` non vide mais NON parseable est fail-closed :
  la campagne n'est jamais rendue publique ;
...
- non expirée (`expires_at` absent => jamais expirée ; sinon expirée dès
  `now >= expires_at` — comparaison UTC déterministe, naive traitée comme UTC) ;
...
def _parse_date(value) -> Optional[datetime]:
    """Parse une date 'YYYY-MM-DD' stockée en chaîne. Retourne une datetime
    naive, ou None si vide/absente/invalide. L'appelant doit distinguer
    « absent » de « invalide » via `_has_nonempty_date` pour fail-closed."""
    if value is None:
        return None
    if isinstance(value, datetime):
        return value
...
def is_job_expired(job: dict, now: Optional[datetime] = None) -> bool:
    ...
    Comparaison UTC déterministe (naive/aware normalisées)."""
...
def is_job_publicly_visible(job, campaign: Optional[dict] = None,
    ...
    sans campagne résolue (ou campagne non diffusible), l'offre rattachée est
    considérée non visible. Les offres sans `campaign_id` (legacy) restent
...
async def fetch_public_job_filter(db, now: Optional[datetime] = None) -> dict:
    """Filtre Mongo « offres publiquement visibles » (réutilise exactement la
    sémantique de `is_job_publicly_visible`).

    On préfiltre les campagnes plausibles (Mongo), on applique
    `is_campaign_diffusible` (Python) pour obtenir l'ensemble EXACT des
    campagnes diffusibles, puis on construit le filtre sur les offres :
    actives + non expirées + (rattachées à une campagne diffusible OU sans
    campagne du tout — `campaign_id` absent OU `None`, pour préserver les
    offres legacy/non partenaires).
    """
    ts = _now(now)
